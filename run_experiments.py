...
for name, layers, lr, batch, epochs in configs:
    ...
    ax[0].plot(history['train_loss'], label=f'{name} train')
    ax[1].plot(history['train_acc'], label=f'{name} train')
    ax[1].plot(history['val_acc'], '--', label=f'{name} val')
...
ax[1].legend()
fig.tight_layout()
fig.savefig('results/curvas_treino.png')
plt.close(fig)
...
for ax, idx in zip(axes.flatten(), selected):
    ...
    ax.axis('off')
plt.tight_layout()
fig.savefig('results/exemplos_erro.png')
plt.close(fig)

print('Saved results/comparacao_experimentos.csv, results/curvas_treino.png, results/confusion_matrix.png, results/exemplos_erro.png')
